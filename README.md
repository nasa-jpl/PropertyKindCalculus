...
| `PropertyKindCalculus.AggregationLaws` (in the `Dimension` lib) — the two aggregation laws whose arithmetic the Mathlib-free core cannot state: the **weighted mean** over `ℝ` carrying its nonzero-total license as a field (`WeightedCarving.mean_const`), and the **parallel-axis theorem** as the general transport law, its correction built from the carving's own first moment and total mass | — | ✅ |
