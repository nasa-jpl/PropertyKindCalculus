...
fraction"_ (volumetric water content).

This is the construct the QUDV → OML/OWL2 lineage cannot express. A description
logic can record that a quantity _has_ a system and _has_ a component, but it
cannot make _"Soil — Water ; volume fraction"_ and _"Soil — Water ; mass
fraction"_ provably distinct kinds _while keeping both dedicated to the same system
...
A dedicated kind joins three things the spine already carries — a {uses "def_system"}[system] (Ch. 3), a component, and a {uses "def_kindOfProperty"}[kind-of-property] (Ch. 6) — into the IUPAC/IFCC `System — Component ; kind` designation. It is a Mathlib-free core module; the worked example _"Soil — Water ; volume fraction"_ versus _"… ; mass fraction"_ is checked in the `Examples` library.
:::

:::definition "def_component" (parent := "spine_dedicated") (lean := "PropertyKindCalculus.Component")
A _component_ (§20) is the pertinent component of the {uses "def_system"}[system] a dedicated kind is about — _water_ in a _soil_ sample, _glucose_ in _plasma_. Specified abstractly by identity, exactly mirroring the system carrier.
:::
...
*Same system and component, a different kind-of-property: a distinct dedicated kind.* This is the principled volumetric-versus-gravimetric distinctness. Both water contents are _"Soil — Water"_, but volume fraction and mass fraction are different {uses "def_kindOfProperty"}[kinds-of-property], so the {uses "def_dedicatedKind"}[dedicated kinds] differ — with the system and component held identical, and without appeal to any hand-chosen identity string. This is exactly the discrimination an OWL2 reasoner cannot phrase.
:::

:::proof "thm_dedicated_distinct_kind"
Assume the two dedicated kinds are equal; then their `kind` projections are equal by congruence, contradicting the hypothesis (`intro he; apply h; rw [he]`). The companion laws `distinct_of_system` and `distinct_of_component` are the same one-line argument on the other two projections.
:::
...
*Same system and kind, a different component: a distinct dedicated kind.* The water content and the air content of one soil sample share their {uses "def_system"}[system] and their {uses "def_kindOfProperty"}[kind-of-property], differing only in the {uses "def_component"}[component] — and that alone makes them distinct {uses "def_dedicatedKind"}[dedicated kinds].
:::
