...
example : lengthKind.toDimension ≠ vwc.toDimension := by
  intro h
  have : (Dim.length).length = (1 : Dimension LTMCTDimensionBase).length := congrArg Dimension.length h
...
example : (DimensionedKind.unitless (B := LTMCTDimensionBase)).toDimension = 1 :=
