...
sorry-free axiom profile.
  ...
  * **`round32_sterbenz`.** For representable binary32 `u`, `v` within a factor of two, the exact
    difference is already on the binary32 grid — `round₃₂ (u − v) = u − v`. Symbolic in `u`, `v`
  ...
  * **`sub32_error_vanishes`.** The `sub32_within_half_ulp` bound (`≤ eps₃₂`) collapses to *exactly*
...
/-- **Sterbenz over `round₃₂`.** For representable binary32 `u`, `v` within a factor of two, the exact
difference is already on the binary32 grid — so `round₃₂` is the identity on it. -/
...
error by `eps₃₂`; in the Sterbenz regime that error is *exactly* `0`. -/
