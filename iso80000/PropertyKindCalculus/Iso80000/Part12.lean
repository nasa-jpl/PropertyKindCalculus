...
def vibrationalDOS : Dimension LTMCTDimensionBase := numberDensity * Dim.time
...
def resistivity : Dimension LTMCTDimensionBase := voltage / Dim.current * Dim.length
/-- Energy density of states, `M⁻¹·L⁻⁵·T²` (`J⁻¹·m⁻³`). -/
def energyDOS : Dimension LTMCTDimensionBase := (Dim.energy * volume)⁻¹
/-- Lorenz coefficient, `M²·L⁴·T⁻⁴·C⁻²·Θ⁻²` (`V²/K²`). -/
def lorenz : Dimension LTMCTDimensionBase := (voltage * voltage) / (Dim.temperature * Dim.temperature)
/-- Hall coefficient, `L³·C⁻¹` (`m³/C`). -/
def hallCoeff : Dimension LTMCTDimensionBase := volume / Dim.charge
/-- Seebeck and Thomson coefficient, `M·L²·T⁻²·C⁻¹·Θ⁻¹` (`V/K`). -/
def seebeck : Dimension LTMCTDimensionBase := voltage / Dim.temperature
/-- Richardson constant, `C·T⁻¹·L⁻²·Θ⁻²` (`A·m⁻²·K⁻²`). -/
def richardson : Dimension LTMCTDimensionBase := Dim.current / (Dim.area * Dim.temperature * Dim.temperature)
/-- Magnetic flux density, `M·T⁻¹·C⁻¹` (`T`, the tesla) — the critical flux densities. -/
def magFluxDensity : Dimension LTMCTDimensionBase := (Dim.energy * Dim.time) / (Dim.charge * Dim.area)
