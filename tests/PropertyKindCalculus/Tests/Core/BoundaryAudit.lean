...
(`@[kindCrossing]`, `@[carrierVocab]`, `@[kindEmission]`), one **untagged** mint (the
violation the audit must catch), a clean pass-through (which must *not* register), and a
downstream carrier registered through `@[kindCarrier]`, then pins the sorted report and the
crossings enumeration.
-/
...
5 boundary site(s): 4 tagged, 1 UNTAGGED — invariant 6/7 violation
-/
#guard_msgs in
