...
that `lake build` does not verify on its own, for **every** verifiable requirement (the twenty
among R1–R27):
...
Organized by dependency tier, matching the layering of the requirement catalogue:
  ...
  * `Tests.Core` — the Mathlib-free core spine (R2, R3, R4, R6, R8, R9, R10, R11, R12, R16, R17,
    R19, R21, R22, R27), grouped by the catalogue's `RequirementGroup`;
  * `Tests.Dimension` — the PhysLib/Mathlib-backed dimension layer (R1, R5, R7, R13, R17-ℝ, R20);
...
twenty verifiable requirements are CI-enforced, not just those in the core spine.
-/

import PropertyKindCalculus.Tests.Core
import PropertyKindCalculus.Tests.Dimension
import PropertyKindCalculus.Tests.Iso80000
