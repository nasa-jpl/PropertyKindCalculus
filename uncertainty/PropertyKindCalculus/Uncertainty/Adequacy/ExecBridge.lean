...
artificial spacing) and states the capstone:
  ...
  * `exec_ulp_grounds` / `exec_half_ulp_grounds` — when the *executable* ULP query answers
    ...
    is exactly the specified `ulp₃₂` / `eps₃₂` of the decoded value — and the query answers on
    precisely the finite fragment (`ulpExp?_isSome`). The `eps₃₂` form is the very half-ulp
...
(`ulpExp?_isSome`) — then `2^k` reproduces exactly the real, `noncomputable` `ulp₃₂ (toReal x)`.
The first half of the "`Float.ulp` matching `ulp₃₂`" the Stage-3.3 plan asks for, on the
...
theorem exec_half_ulp_grounds {x : IEEE32Exec} {k : Int} (hk : ulpExp? x = some k) :
    neuralBpow binaryRadix k / 2 = eps₃₂ (toReal x) := by
