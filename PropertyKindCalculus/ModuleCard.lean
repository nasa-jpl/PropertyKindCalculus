...
open PropertyKindCalculus.Provenance (Contract Port PortDir)
...
discharge line). -/
def moduleCard (c : Contract String String)
  ...
  if !groups.isEmpty then
    ...
    out := out ++ "    style: {stroke: \"#9ca3af\"; fill: \"#ffffff\"; border-radius: 8; font-size: 14; bold: true}\n"
    let deciderOf (n : String) : Option String :=
    ...
    for (glabel, ps) in groups do
      ...
      let mut i := 0
      for p in ps do
        let a := shortAddr p.node
        let disp := if (ps.filter (fun p' => shortAddr p'.node == a)).length == 1
          then a else p.node
        let mark := if c.exits.contains p.node then " ⊗" else ""
        let tip := match deciderOf p.node, disp == p.node with
          | some d, _ => some s!"decided by {d} · {p.node}"
          | none, false => some p.node
          | none, true => none
        let row : ClauseRow :=
          ⟨s!"{disp} : {p.kind}{mark}", portFill p.dir, portStroke p.dir, tip⟩
  ...
  let clauseRows : List ClauseRow :=
    c.aggregations.map (fun (n, a) =>
      ⟨s!"aggregates {shortAddr n}: {a.label}", clauseFill, clauseStroke, some n⟩)
    ++ c.suppliers.map (fun (n, s) =>
      ⟨s!"supplies {shortAddr n}: {shortAddr s}", clauseFill, clauseStroke,
        some s!"{n} ← {s}"⟩)
    ++ c.exits.map (fun n =>
      ⟨s!"exit {shortAddr n} ⊗ — leaves the calculus", clauseFill, clauseStroke, some n⟩)
