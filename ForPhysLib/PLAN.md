...
| The Marmodoro pass, Options 1–3 of [the corpus](MARMODORO-CORPUS.md), decided 2026-09-04: (1) the unite/unify layering stated and cited — a verified `InProceedings` entry for ch. 4 of the vendored *Themes* volume, the carving paragraph in the *Extensivity* chapter and `Extensivity.lean`, the sortal paragraph in the blueprint's foundations chapter (moved there 2026-09-05; originally in *The object type*) and in `Composite.lean`/`Foundations.lean`; (2) two more aggregation modes formalized — `Intensive` (§13.5.4, carrying Dybkær's *of constant composition* as the law's hypothesis) + the `intensive_uniform` capstone + `not_extensive_of_intensive`, and `WholeProper` + its two refutations + `assemble_ne_measured`, with density and normal-mode witnesses; (3) Exhibit G | PKC core `Extensivity`/`Composite`/`Foundations`; blueprint *Extensivity* (five new nodes) + *The object type*; `Tests/Core/{Aggregation,Composite}`, `MiniExtensivity`; `Exhibits/Composition/` |
| The Marmodoro pass, Option 4 of [the corpus](MARMODORO-CORPUS.md), decided 2026-09-04 with both consumers named: the re-carving construct (`Recarving m` — a map between carvings of one whole plus the obligation that the whole is preserved) with `Recarving.leafSum_invariant`, which states once, for any extensive kind, what `TenQuanta.reduce`/`reduce_sum_eq_sum_toCharges` proves one additive functional at a time; and count kinds (`Decomposition.count`/`countMeasurement`, keyed by a sortal), with the count extensive over a fixed carving (`dof_add`'s law) and yet `coalesce_count_ne` refuting that it is a property of the whole — the SI's "count of a specified elementary entity" as the reason a sortal is needed | PKC core `Recarving`; blueprint *Extensivity* (three new nodes); `Tests/Core/Recarving`; `@[requirement "R9"]` annotations |
| The §13.5 completion pass: §13.5.2 formalized (`Uncertainty.QuasiExtensive` — a named per-join tolerance, the join-count bound over a whole carving, §13.5.1 recovered at zero tolerance, and `join_within_tolerance` sourcing the tolerance from R18's Chebyshev bound); extensivity about a shared parameter (`ExtensiveAbout`/`Transports`) with the parallel-axis theorem as the general transport law (`AggregationLaws.parallelAxis`); the weighted mean promoted out of Exhibit G into `AggregationLaws` beside the other two modes; and the real-valued chosen-reference unit `RealUnit` | PKC core `Extensivity`/`Recarving`, `Dimension` `AggregationLaws`/`UnitReal`, `UncertaintyRigor` `QuasiExtensive`; blueprint *Extensivity* + *Units* (`def_unit` realized); `Tests/{Core,Dimension,Uncertainty}` |
| The Lowe correspondence, explicit and cited: corners in each docstring, edges + the derivative diagonal in the blueprint's foundations chapter (moved there 2026-09-05; originally in *The object type*); formal entry from Nicolas's verified BibTeX (OUP 2005, DOI) in `References.lean`; two chapter entries from the vendored *Ontology, Modality, and Mind* (OUP 2018) — Simons (exemplification definable; cited at the diagonal) and Heil (Lowe's immanentism; cited for the universals-as-catalogue-entries stance). Marmodoro's "Whole, but not One" flagged as a possible future pass for `Composite`/`Assembles` (unite vs unify; a whole carries no count principle) | blueprint `References.lean` + *The object type*; year corrected 2006→2005 per the verified citation |

| The foundation brought upfront, decided 2026-09-05: the blueprint's foundation stated as Dybkær + Lowe, with a new chapter *Foundations: system, part, and the ontological square* second in render order — Dybkær's implied mereology documented against the vendored PDF (the partitive relation among concepts: Fig. 2.1, Fig. 6.9, §9.13.2, Fig. 9.25, §11.8; part-talk among systems: §3.2, §3.3 + Notes 3/5, §3.4 + Notes 2/3, Fig. 3.5, §5.5.1; the §2.23.3 bundle-theory refusal as the license for Lowe), the square + immanentism + unite/unify prose moved there from *The object type*, `def_system` moved from the spine, and the carving (`Decomposition`) + `Part`/`Whole` roles given foundation nodes; *Extensivity* and *The object type* now cite backwards | blueprint `Chapters/Foundations.lean`; *The Proved Spine*, *The object type*, *Extensivity*, *Dedicated kinds-of-property* adjusted; `blueprint/README.md` (26 chapters) |
